    ...
    for (let l = 0; l < contagemDeBlocosEmLinha; l++) {
      ...
      if (bloco.estado === 1) {
        if (
          ...
          if (pontos == (contagemDeBlocosEmLinha * contagemDeBlocosEmColuna) * dificuldade) {
            alert('Você passou de nivel, parabéns')
            aumentaDificuldade()
          }
        }
      }
...
function desenhaPontos() {
  ...
  context.fillText(`Dificuldade: ${dificuldade}`, 140, 30)
}
